...
const transformProduct = (p: DBProduct) => ({
  ...
  currency: p.currency,
  images: p.images || [],
  media: (p.specs && (p.specs as any).media) || p.images || [],
...
const ProductDetail = () => {
  ...
  const nextImage = () => {
    if (!product) return;
    setSelectedMediaIndex((prev) => (prev + 1) % product.media.length);
  };

  const prevImage = () => {
    if (!product) return;
    setSelectedMediaIndex((prev) => (prev - 1 + product.media.length) % product.media.length);
  };
              ...
              <div className="space-y-4">
                <div className="relative aspect-square bg-secondary rounded-2xl overflow-hidden">
                  {product.media && product.media.length > 0 ? (
                    product.media[selectedMediaIndex].type === 'video' ? (
                      <video controls className="w-full h-full object-contain p-8 bg-black">
                        <source src={product.media[selectedMediaIndex].url} />
                        Your browser does not support the video tag.
                      </video>
                    ) : (
                      <img
                        src={product.media[selectedMediaIndex].url || "/placeholder.svg"}
                        alt={product.name}
                    ...
                    <img
                      src={product.images[selectedMediaIndex] || "/placeholder.svg"}
                      alt={product.name}
                      className="w-full h-full object-contain p-8"
                    />
                  )}
                ...
                {/* Thumbnails */}
                  ...
                  <div className="flex gap-3">
                    {product.media.map((m: any, index: number) => (
                      <button
                        key={index}
                ...
                {product.inStock && !product.isUpcoming && (
                  <div className="flex flex-col sm:flex-row gap-4 mb-8">
                    ...
                    <Button variant="outline" size="sm" onClick={() => {
                      ...
                      const refParam = code ? `?ref=${code}` : '';
                      const url = `${window.location.origin}/product/${product.slug}${refParam}`;
                      ...
                      const copy = async (text: string) => {
                        try {
                          ...
                          el.select();
                          try { document.execCommand('copy'); document.body.removeChild(el); return true; } catch (err) { document.body.removeChild(el); return false; }
                        }
                      };
                      copy(url).then((ok) => { if (ok) alert('Share link copied'); else alert('Could not copy link') });
                    }}>Share</Button>
