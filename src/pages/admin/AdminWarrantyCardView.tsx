...
export default function AdminWarrantyCardView() {
  ...
  const fetchCard = async (cardId: string) => {
    ...
    try {
      ...
      if (data?.product_serial) {
        const { data: s } = await supabase.from('serial_numbers').select('*').eq('serial', data.product_serial).maybeSingle();
        setSerial(s || null);
      } else setSerial(null);
      if (data?.created_by) {
  ...
  const handleSyncSerial = async () => {
    if (!card?.product_serial) return;
    try {
      const warrantyEnd = card.warranty_end_date || null;
      const payload = {
        serial: card.product_serial,
        product_name: card.product_name,
        is_sold: true,
        sold_at: card.purchase_date,
        warranty_start_date: card.purchase_date,
        warranty_end_date: warrantyEnd,
        warranty_period: card.warranty_period || 6,
        created_by: card.created_by || null,
      };
      const { error } = await supabase.from('serial_numbers').upsert(payload, { onConflict: 'serial' });
      if (error) throw error;
      const { data: s } = await supabase.from('serial_numbers').select('*').eq('serial', card.product_serial).maybeSingle();
      setSerial(s || null);
      alert('Serial synced to serial_numbers.');
    } catch (err: any) {
      console.error(err);
      alert('Failed to sync serial: ' + (err.message || err));
    }
  };
    ...
    <div className="max-w-3xl mx-auto p-6 bg-background rounded-lg shadow">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {card.product_serial && (
            <Button variant="outline" onClick={handleSyncSerial}>
              Sync Serial
            </Button>
          )}
        </div>
          ...
          <div>
            ...
            {serial && (
              <div className="mt-2 p-2 border rounded text-sm">
                <div>Serial Status: <strong>{serial.status || (serial.is_sold ? 'sold' : 'available')}</strong></div>
                {serial.warranty_end_date && <div>Warranty Until: <strong>{format(new Date(serial.warranty_end_date), 'MMM dd, yyyy')}</strong></div>}
              </div>
            )}
          </div>
        </div>
