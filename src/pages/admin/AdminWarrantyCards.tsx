...
export function AdminWarrantyCards() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If a serial number was provided, upsert it into serial_numbers
      try {
        if (formData.product_serial && formData.product_serial.trim() !== "") {
          const serialPayload = {
            serial: formData.product_serial.trim(),
            product_name: formData.product_name,
            is_sold: true,
            sold_at: formData.purchase_date,
          ...
          const { error: serialError } = await supabase
            .from("serial_numbers")
            .upsert(serialPayload, { onConflict: "serial" });

          if (serialError) console.warn("Failed to upsert serial number:", serialError.message);
        }
      } catch (err) {
        console.error("Error upserting serial number:", err);
      }
